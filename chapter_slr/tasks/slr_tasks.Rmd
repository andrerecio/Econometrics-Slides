...
What is a regression without any intercept? In other words, $b_0 = 0$. To generate some intuition, let's get one of R's built-in datasets and have a look:
...
```{r}
b1 = sum(cars$speed * cars$dist) / sum((cars$dist)^2)
b0 = 0
```
...
We then run a regression *without intercept* as above. That is, we use $\tilde{x}_i,\tilde{y}_i$ instead of $x_i,y_i$ to obtain our slope estimate $b_1$:
...
We can use the same formula from above, because if we standardize both left and rigiht hand side in this here

$$y_i = b_0 + b_1 x_i + e_i$$

then $b_0$ drops out! So, we have
