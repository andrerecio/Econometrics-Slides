...
# What's the proportion of green noodles?
...
We could count every green noodle but that would be tedious! 😟 What else could we do?
...
.pull-left[
* Let's take a sample of 20 noodles.
...
* `r format(signif(14/20,2), nsmall = 2)` can be thought of as our guess of the proportion of green noodles in the entire bowl.
]
...
* What would happen if we took a *new* sample (putting the 20 previous noodles back in the bowl)? Would we also get 14 *greens* as before?
...
* Because we don't have noodles with us in class, we drew 18 samples of 20 noodles (with replacement) at home.
...
* For each sample, we computed the share of green noodles.
...
1. Create a data.frame containing the proportions of green noodles from the previous slide. Name it `pasta` and name the variable containing the proportions `prop_green`. (Hint: to create a data.frame you need to use the `data.frame()` function.)
The proportions are as follows: `(0.7,0.7,0.5,0.5,0.3,0.5,0.4,0.45,0.55,0.4,0.35,0.45,0.45,0.7,0.55,0.5,0.35,0.65)`
...
* *Objective*: know the proportion of green noodles
...
--
  ...
  1. **Sampling**: extract a *sample* of 20 noodles from the bowl to obtain an ***estimate***.  
  Out first ***estimate*** of the proportion of green noodles was 0.70, but it was actually larger than most other ***estimates***.
...
.pull-left[
* We counted the exact number of green, red and yellow noodles in the bowl 🧐 `#lockdown`

* All the noodles in the bowl are stored in a csv file [here](https://www.dropbox.com/s/qpjsk0rfgc0gx80/pasta.csv?dl=1).

```{r, echo = TRUE,}
bowl <- read.csv("https://www.dropbox.com/s/qpjsk0rfgc0gx80/pasta.csv?dl=1")

kable(head(bowl, n = 4)) # first 4 rows
```
...
* Instead of selecting noodles with our hands, we'll take *virtual* draws from the bowl.

* We'll use the *virtual shovel* to take a sample of 50 noodles from our virtual bowl.
]
...
virtual_shovel <- bowl %>% # notice that moderndive functions can be "pipped"
  rep_sample_n(size = 50) # take a sample of 50 noodles
```
...
# Proportion of Green noodles

.pull-left[
```{r}
sample_1 <- virtual_shovel %>% 
  summarize(
    # number of green noodles in sample
    num_green = sum(color == "green"),
    # number of observations in sample
    sample_n = n()) %>% 
  mutate(
    # proportion of green noodles in sample
...
.pull-right[
1. Compute:
  * sum of green noodles in sample,
  * number of observations in sample (i.e. 50 in this case)

1. Compute proportion of green noodles

👉 `r sample_1$prop_green` are green! This is an ***estimate*** of the proportion of green noodles in the bowl. What if we try again?
...
Compute the proportion of green noodles in each sample.
...
ggplot(virtual_prop_green, aes(x = prop_green)) +
  ...
  labs(x = "Proportion of 50 noodles that were green",
       y = "Frequency",
...
1. Calculate the proportion of green noodles in each sample.

1. Plot a histogram of the obtained proportion of green noodles in each sample.
...
1. How likely is it that we sample 50 noodles of which less than 20% are green?
...
virtual_prop_green %>% ggplot(
  ...
  geom_histogram(
    ...
    color = "white",
    fill = "darkgreen") +
  labs(x = "Proportion of green noodles in sample",
       y = "Frequency",
...
If your goal is still to estimate the proportion of the bowl’s noodles that are green, which shovel would you choose?
...
Compute proportion of green noodles:
...
df %>% 
  ...
  labs(
    x = "Proportion of green noodles in sample",
    y = "Frequency",
    title = "Comparing distributions of proportions of green noodles for different sample sizes"
  ) +
...
* So as the sample size increases, our ***estimates*** of the true proportion of the bowl's green noodles get more *precise*.
...
* We extracted samples in order to ***estimate*** the proportion of the bowl's noodles that are green.
...
*Examples:* population mean $(\mu)$, proportion of green noodles $(p)$.
...
*Example:* *sample proportion* of green noodles $(\hat{p})$. The "hat" on top of the $p$ indicates that it is an *estimate* of the population proportion $p$.

* ***Representative sampling:*** does the sample *look like* the population?

* ***Biased sampling:*** did all noodles have an equal chance of being included in a sample?
...
* What is the true population proportion $p$ of green noodles in the population of $N=713$ noodles?
...
* We have discussed this in the early lectures of this semester.
