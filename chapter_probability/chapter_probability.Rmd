...
```{r}
data("gapminder", package = "dslabs")  # reload gapminder data
...
(jd <- prop.table(abstab))  # proportions in each cell
```
...
![:scale 70%](../img/photos/table2-3.png)
...
> The average height is `r mean(df$height)`
...
```{r}
2/5 * 1.635+ 3/5 * 1.836667
```

* What did we just do?

$$\Pr(men) \times \text{average height men} + \Pr(women) \times \text{average height women} = 0.43 * 1.86 + (1-0.43) * 1.59 = 1.7091$$
