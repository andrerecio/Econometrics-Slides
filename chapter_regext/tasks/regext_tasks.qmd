---
title: "Simple Regression Extensions - Tasks"
...
gm$omit[gm$raw == 'r2.adjusted'] <- FALSE
```

# Task: Wages or Log Wages?


1. install the "wooldridge" package via `install.packages("wooldridge")`. Check out [Jeffrey Wooldridge's](https://econ.msu.edu/about/directory/wooldridge-jeffrey) website. 
...
3. Make the same plot but use `log(wage) ~ educ` as your formula instead. Observe what?
...
4. Run 3 regression models: `wage ~ educ, log(wage) ~ educ, log(wage) ~ log(educ)`. Take care with the last model!
...
## Task: Wages, education and gender in 1985
