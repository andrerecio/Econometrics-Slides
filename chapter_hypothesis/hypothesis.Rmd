...
* not the only option: could also have used the average of the largest two values, like $\tilde{y} = \frac{max(y) - min(y)}{2}$, no? Or what about the *last* value in the dataset, $y_n$?

* generically, we call an estimator for population quantity $x$ often $\hat{x}$.
...
then we say that $\hat{x}_2$ is *more efficient* than $\hat{x}_1$ - because uses the same amount of data in a more efficient way. 
