...
\begin{align}
\text{police} &= \alpha_0 + \alpha_1 \text{crime} + u\\
\text{crime} &= \beta_0   + \beta_1  \text{police} + e
\end{align}
...
```{r cri-dag,echo = FALSE,message = FALSE,fig.height=8,fig.width = 10}
library(ggdag)
coords <- list(
...
pcolor = css %>% 
  ...
  geom_point(aes(color = factor(county)),size = 3) + 
  mytheme +
  geom_smooth(method = "lm", se=FALSE,linewidth=2) +
  labs(x = 'Probability of Arrest', 
       y = 'Crime Rate',
...
```{r,echo = FALSE,fig.height = 5,warning = FALSE,message = FALSE,fig.width=10}
p1 = css %>% 
  group_by(county) %>%
  mutate(label = case_when(
    crmrte == max(crmrte) ~ paste('County',county),
