...
```{r plane3D-lwage,echo=FALSE,fig.width=12,fig.height=9}
# data("wage1", package = "wooldridge")
# linear fit
fit <- lm(avgmath ~ classize + disadvantaged , data=grades)
...
surf <- acast(df, classize ~ disadvantaged)

color <- rep(0, length(df))
p1 = grades %>%
  plot_ly(colors = "blue") %>%
  add_markers(x = ~classize, y = ~disadvantaged, z = ~avgmath,name = "data",opacity = .8, marker=list(color = 'red', size = 5, hoverinfo="skip", opacity = 0.5)) %>%
  add_surface(x = to_plot_x, y = to_plot_y, z = ~surf, inherit = F, name = "Avgmath score", opacity = .75, cauto = F, surfacecolor = color) %>%
  hide_colorbar() %>%
...
```{r plane3D,echo=FALSE,fig.width=12,fig.height=9}
# linear fit
...
surf2 <- acast(df, wt ~ hp)

color <- rep(0, length(df))
p2 = mtcars %>%
  plot_ly(colors = "grey") %>%
  add_markers(x = ~wt, y = ~hp, z = ~mpg,name = "data",opacity = .8, marker=list(color = 'red', size = 5, hoverinfo="skip")) %>%
  add_surface(x = to_plot_x, y = to_plot_y, z = ~surf2, inherit = FALSE, name = "Mtcars 3D", opacity = .75, cauto = F, surfacecolor = color) %>%
  hide_colorbar() %>%
    layout(showlegend = FALSE)
p2
```
...
# Least Squares Assumptions In Multiple Regression Model

We need 4 conditions for OLS to yield unbiased estimates:

1. $E[u | X_{i,1}, X_{i,2}, \dots, X_{i,k}] = 0$: The conditional mean of the population regression function ($u$) is uncorrelated to $X$. In other words, all unobserved factors - absent from the model and lumped into $u$ - are *not* systematically related to the explanatory variable $X$.

2. $\left\{X_{i,1}, X_{i,2}, \dots, X_{i,k},Y_i\right\}_{i=1}^n$ is an i.i.d. sample. There is no dependence between observations (like in time series data for example)

3. There are no large outliers in the data.

4. There is no *perfect* multicollinearity: each added regressor adds *at least some* new information. 
...
When you have an omitted variable problem, the assumption that $E(u_i | X_i) = 0$ is violated. This implies that:

1. the sum of the residuals is no longer zero.
2. there is another estimator called weighted least squares, which is BLUE.
3. the sum of the residuals times any of the explanatory variables is no longer zero.
4. the OLS estimator is no longer consistent.
...
1. should be excluded if one explanatory variable has negative values.
2. determines the height of the regression line.
3. should be excluded because the population regression function does not go through the origin.
4. is statistically significant if it is larger than 1.96.
...
1. minimizing the sum of squared prediction mistakes.
1. setting the sum of squared errors equal to zero.
1. minimizing the absolute difference of the residuals.
1. forcing the smallest distance between the actual and fitted values.
...
1. will always be present as long as the regression R2 < 1.
1. is always there but is negligible in almost all economic examples.
1. exists if the omitted variable is correlated with the included regressor but is not a determinant of the dependent variable.
1. exists if the omitted variable is correlated with the included regressor and is a determinant of the dependent variable.
...
1. if X1 and X2 are correlated
1. always
1. if X2 is measured in percentages
1. if X2 is a dummy variable
