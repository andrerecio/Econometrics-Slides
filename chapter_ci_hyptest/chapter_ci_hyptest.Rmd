...
* As in real life, imagine we had access to *only one random sample* from our bowl of noodles.
...
The proportion of green noodles in this sample is: $\hat{p} = `r p_hat`$.
]

---

# Resampling from our Noodles Sample

How do we obtain a ***bootstrap sample***?

--
  
  1. Randomly pick ***one*** noodle from the sample and record the associated color.
  
--
  
  1. Put this noodle **back** into sample.
...
--
  ...
  1. Compute the proportion of green noodles in the bootstrap sample.
...
This procedure is called ***resampling with replacement***:
  ...
  * *with replacement*: each time the drawn noodle is put back in the sample.

---

# Resampling our Noodles Sample
...
Several noodles have been drawn multiple times. How come?

What's the proportion of green noodles in this bootstrap sample?
...
bootstrap_distrib = my_sample %>%
  ...
  generate(reps = 1000, type = "bootstrap") %>% 
  # calculate the proportion of green noodles for each
  calculate(stat = "prop") 
```
]
...
boot_distrib_plot = bootstrap_distrib %>%
  ...
  labs(x = "Proportion of green noodles",
       y = "Frequency") +
...
* In our case the fish is the true proportion of noodles in the bowl that are green $(p)$.

--

* The *point estimate* would be the proportion of green noodles obtained from a random sample $(\hat{p})$.
...
ggplot(percentile_cis) +
    ...
    labs(x = expression("Proportion of green noodles"), y = "Confidence interval number", 
