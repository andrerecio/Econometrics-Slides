...
* Theory-based approach uses one fundamental result: the sampling distribution of the sample statistic $\frac{b - \beta}{\hat{\textrm{SE}(b)}}$ *converges* to a ***standard normal distribution*** as the sample size gets larger and larger.
  ...
  * It is also obtained through a theoretical formula (which you can find in the [book](https://scpoecon.github.io/Econometrics/std-errors.html#se-theory)!) but we'll leave it aside.
...
* Before delving into these assumptions, let's see the small but important modifications we apply to our model (back to [*lecture SLR*](https://raw.githack.com/ScPoEcon/Econometrics-Slides/master/chapter_slr/chapter_slr.html)):
